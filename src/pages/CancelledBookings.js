...
function CancelledBookingRow({ booking, isSelected, onSelect, onNavigateToDetail, onRefresh }) {
  const [showActions, setShowActions] = useState(false);

  const handleReactivate = async (e) => {
    e?.stopPropagation();
    
    if (!confirm('Are you sure you want to reactivate this booking?')) return;
    
    try {
      // Update the booking to mark it as not cancelled
      ...
      alert(`Failed to reactivate booking: ${error.message}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 mr-4 min-w-0">
        <h3 className="text-sm font-semibold text-gray-500 line-through truncate mb-1">
        ...
        <div className="flex items-center gap-4">
          {booking.location && (
            <div className="flex items-center gap-1">
              ...
              <p className="text-xs text-gray-400">{booking.location.name || 'Unknown'}</p>
              <Globe className="w-3 h-3 text-gray-400 ml-1" />
              <p className="text-xs text-gray-400">{booking.location.time_zone || 'UTC'}</p>
            </div>
          )}
          <div className="flex items-center gap-1">
            <User className="w-3 h-3 text-gray-400" />
            <p className="text-xs text-gray-400">{booking.metadata?.customer_name || booking.customer_name || booking.customer || 'Unknown'}</p>
          </div>
        </div>
      </div>

      <div className="w-32 mr-4 text-sm text-gray-500">
        {booking.price > 0 ? `$${parseFloat(booking.price).toFixed(2)}` : '-'}
      </div>

      <div className="w-24 mr-4 text-sm text-gray-500">
...
export default function CancelledBookings() {
  ...
  const fetchCancelledBookings = async () => {
    try {
      ...
      // Fetch all bookings - single API call
      ...
      // Filter cancelled bookings using is_canceled flag or canceled_at field
      const cancelledBookings = (data.data || data || []).filter(booking => 
        booking.is_canceled === true || booking.canceled_at !== null
      );
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Delete ${selectedBookings.length} bookings permanently?`)) {
            ...
            <div className="relative">
              ...
              {showBulkActions && (
                <div className="absolute top-full right-0 mt-1 bg-white border rounded-md shadow-lg z-10 min-w-48">
                  <button className="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50" onClick={handleBulkDelete}>Delete Selected</button>
                </div>
              )}
            </div>
          )}
      ...
      <div className="bg-white border rounded-lg overflow-hidden">
        ...
        <div>
          {paginatedBookings.length === 0 ? (
            <div className="text-center py-12 text-gray-500">No cancelled bookings found</div>
          ) : (
            paginatedBookings.map((booking) => (
              <CancelledBookingRow 
                ...
                onRefresh={fetchCancelledBookings}
              />
            ))
          )}
        </div>
